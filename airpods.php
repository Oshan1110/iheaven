...
<body class="home-body">
  ...
  <div class="product-container">
    ...
    $category_rs = $conn->query("SELECT * FROM `category` WHERE `cat_id` = 6");
    ...
    if ($category_rs->num_rows > 0) {
      while ($category_data = $category_rs->fetch_assoc()) {
        ...
        $product_rs = "SELECT * FROM `product` INNER JOIN `stock` ON `product`.`id` = `stock`.`product_id` WHERE `category_cat_id`='$category_id' ORDER BY `datetime_added`";
